...
As reported in the state of the art, Java is one of the most popular programming languages adopted by practitioners.%ROMAIN: Add source
Furthermore, if we take into consideration legacy applications, Java becomes the most used programming langues.%ROMAIN: Add source
In addition to its popularity, Java exhibits an interesting behavior when it comes to energy consumption and performances, basically Java applications can be at the same time one of the most energy efficient or hungry solution.
...
\begin{compactenum}[\indent\bf RQ\,1:]
    %  TODO : generalize this format of Rq  to all the thesis 
...
\subsection{definition of the JVM}
...
\Cref{fig:jvm-benchmarks-classification} highlights the scope of each benchmark from the test suite.
...
For example, the introduction of \texttt{VarHandle} to allow low-level access to the memory order modes available in JDK 9 and work along \texttt{Unsafe Class} that was removed from from JVM 11.\footnote{\url{https://blogs.oracle.com/javamagazine/the-unsafe-class-unsafe-at-any-speed}}
...
The tool was tested for 2 real Java projects: \textsf{Zip4J}\footnote{\url{https://github.com/srikanth-lingala/zip4j}} and \textsf{K-nucleotide}.\footnote{\url{https://benchmarksgame-team.pages.debian.net/benchmarksgame/performance/knucleotide.html}}.
...
% TODO : add or change the graphes of the use case 
% TODO : add the description and a diagrame for the experimental protocol  