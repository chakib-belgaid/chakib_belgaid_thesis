...
Then we provide some insights on the impact of programmer choices in the second section. To do so, we will conduct a set of experiments on different Python aspects, from data structures and iteration mechanisms to the impact of training algorithms in machine learning, passing by the impact of the concurrency strategy and the use of orms in web development.
Finally, in Section~\ref{python:sec_interpreters},we investigate other non-intrusive approaches to optimizing the energy consumption of applications by comparing multiple Python implementations, which include alternative interpreters and libraries that are dedicated to optimizing the code without changing its structures, such as ahead-of-time (AOT) compilation and just-in-time (JIT) libraries that are maintained by the community.
...
This part aims to formulate some actionable insights to optimize the energy consumption of Python applications without altering the source code.
Therefore, we start by extending the work of Hasan~\emph{et~al.} and Oliveira~\emph{et~al.} to the context of Python environments
\cite{hasan_energy_2016,oliveira_recommending}.
...
As we know python is a single-threaded program thanks to the GLS (\emph{Global Lock System}) however, due to the increase in the number of cores/threads per CPU, many libraries started to take advantage of this hardware feature by simulating multithreading as multiple instances of the processor. % TODO: TO VERIFY 
...
\clearpage
\section{Summary} 
As Python is widely adopted by software services deployed in public and private cloud infrastructures, Imporving the energy efficiency of python based applications will push the carbon emissions of ICT a step further.
This chapters presents differents approaches to improve the energy efficiency of python based applications. First it presents the reasons behind a such choice, then setup a strategy to improve the energy of the most use cases which revealed to be web developement and machine learning. 
For the web developements, we have shown that most of the energy is consumed while treating requests, hence choosing the right methode can decrease the energy consumption by 10 times. 
After that we, we stdied the energy consumption for the machine learning usecase and we have shown that the energy consumption is not linear with the accuracy of the model. therefore a cut in the accuracy can lead to a significant energy saving. 
After that we went deeper to study how data structures, parallelism and iterating mechanism can affect the energy consumption. 
As for the second section we provided a non intrusive solution to optimize the energy consumption of python based applications. without signigicant changes in the code. which is the use of alternative python interpreters.
Section 2 started by a classification and filtering these implemenation, to come up with three major types (compiler,interpreter and extra libraries).Then we run a set of experiments to compare the energy consumption of these interpreters.
The results shown the absence of a generic solution, and the need to choose the right interpreter for the right use case. Some of these solutions can be the best in somes and at the same time the worst in others. 
Finally we have shown that the most efficient way to increase the energy efficiency of python based applications is the use of JIT, regardless of  implementation. 
We believe that our contributions will benefit a wide diversity of legacy systems and not only favorably contribute to reducing the carbon emissions of ICT but also reduce their cloud invoice for the resources consumed by these services.