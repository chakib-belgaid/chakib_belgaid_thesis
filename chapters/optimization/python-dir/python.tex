...
Despite, the fact that increasing the number of processes will reduce the execution time because we split the tasks on the number of processes, there is a point where the execution time will increase. As one can see in \Cref{fig:python_multiprocessing}, the energy consumption is correlated with the number of processes until reaching the limit of physical cores, when concurrent processes start to compete for the CPU.
