...
This part highlights the work of \citeauthor{kaminetzky_energy_2021}~\cite{kaminetzky_energy_2021} during his internship at inria
...
\begin{figure}
    ...
    \caption{Accuracy along epochs~\cite{kaminetzky_energy_2021}.}
...
\begin{figure}
    ...
    \caption{Cumulative energy consumption vs. accuracy~\cite{kaminetzky_energy_2021}.}
...
\begin{figure}
    ...
    \caption{Average energy for training the model based on the strategy~\cite{kaminetzky_energy_2021}.}
...
\begin{figure}
    ...
    \caption{Evolution of average GPU power along epochs~\cite{kaminetzky_energy_2021}.}
...
\begin{figure}
    ...
    \caption{Distribution of the durations of each epoch~\cite{kaminetzky_energy_2021}.}
...
To accomplish so, we examined the cost of a single request from the prior website using various ways to extract data from the database.
...
This part displays the work of \citeauthor{randrianaina_optimisation_2019}\cite{randrianaina_optimisation_2019} during his internship at Inria.
...
\begin{figure}[!htb]
    ...
    \caption{Comparison of the energy consumption of different Python loops~\cite{randrianaina_optimisation_2019}.}
...
\begin{figure}
    ...
    \caption{Comparison of the energy consumption of different methods to convert a list~\cite{randrianaina_optimisation_2019}.}
...
%  N ==5e5
\begin{itemize}
    \item \textsf{Sequential}: we compute the sum of the first $N$ numbers with no concurrency;
    ...
    \item \textsf{Multiprocessing}: we use the \texttt{ProcessPoolExecutor} library to compute the sum of the first $N$ numbers. We divide $N$ by the number of processes, and each process sums the numbers in its range. In this situation, we considered the 2, 4, 8, and 16 processes iteratively.
\end{itemize}
