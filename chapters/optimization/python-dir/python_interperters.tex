...
Despite the availability of numerous tools for code refactoring and automatic code optimization, the optimization process remains a manual effort. This is due to the fact that code optimization is a task that requires extensive understanding of both the codebase and the language itself.
...
Therefore, nn this section, we will examine at how different Python interpreters affect the energy consumption of software,and in which case, one should use a non-conventional interpreter to save the energy consumption of their application. 
...
We adopted a \textsc{logarithmic scale} to help the practitioners to compare numerous implementations due to the large differences between them.
...
Unlike the previous interpeters, GraalPython exhibits a certain promise when it comes to complex algorithms (nested loops in particular). However, it is still in its early stages of development and it is not yet ready for production. Since some benchmarks required an abnormally lengthy time, such as array manipulation and sorting. 
...
Furthermore, we excluded from this study runtimes that were unable to complete our benchmarks because we judged that this was the bare minimum for an implementation to be employed in a real-world application.
Besides this, since the goal of this study was to provide a non-intrusive technique to decrease the energy consumption of software, we opted to omit some of the implementations that required extra work on the sourcecode to run on them, such as \textsf{Cython}.

\subsection{Conclusion}

One can conclude that the Python interpreter used has a considerable impact on the energy consumption of the programs. Furthermore, the absence of a universal solution makes this invistigation more intriguing.
While Python2 was employed by the majority of the candidates, Python3 provided better compatibility coverage.
Except for Pypy, a standout interpreter, the type of optimization strategy had a substantial impact on energy usage because each class was grouped together regardless of Python version.
Finally The use of JIT, a technology that converts code into machine code at runtime, was responsible for the significant gain in energy efficiency. The usage of translators such as Shedskin confirmed the effect of compiling Python code into machine code. Hence, making PyPy, an interpreter that has a built-in JIT, within the same class as the complied ones such as Shedskin
