...
Most of the approaches were to use the carbon emission, such as~\cite{patterson2021carbon} or ecograder,\link{https://ecograder.com/} as a pivot metric to quantify the environmental cost of the software.
...
This prototype got even more sophisticated to become Joule-diff,\link{https://github.com/davidson-consulting/diff-jjoules} which is a tool to be included in a CI/CD workflow to highlight the energy evolution of the Java programs.
...
\paragraph*{Synthesis}
I believe that the context can have a significant impact on the energy consumption of the system, and sometimes measuring only the energy consumption of an application does not represent its real impact on the energy consumption of the whole system, especially when we are dealing with timeless Software aka services.
In the future, we tend to explore this behavior from different angles, such as the energy consumption of the application itself instead of the whole system using \cite{fieni2020smartwatts}, comparing the strategies that were mentioned in this thesis using multiple contexts instead of just a minimal version of the operating system, and studying the impact of deliberately putting the application on hold when we have a lower CPU frequency to reduce its impact using some opportunistic schedulers.
