...
We are conducting empirical tests to determine the effect of programmer choice on software's energy consumption.
To accomplish this, one should not overlook the benchmarking pitfalls highlighted in the state of the art.
Moreover, when it comes to measuring the energy consumption of software, other obstacles arise, such as the impact of hardware, the difference between the machine used in the research and the machines that may apply the findings, etc. 
Moreover, significant improvements in computer science have resulted in an increase in the quantity of obsolete results. It is even more difficult to address all of them in studies that compare multiple solutions.
Furthermore, there may have been the appearance of new candidates as well as the development of others between the preliminary experiment and the published results. As a result, it is not only required to ensure the "reproducibility" of the results, but also to make way for new candidates. 
As a result, we'd like to propose a new concept for a fruitful experiment, which we'll call "Extension." The capacity to provide the tools required not only to repeat the experiment but also to add additional candidates, workloads, or crucial Key metrics.
...
We propose making certain enhancements to the benchmarking framework that was suggested by the collaboratory on Experimental Evaluation of Software and Systems in Computer Science in order to address those problems \footnote{\url{http://evaluate.inf.usi.ch/}}.
Instead of only presenting the four primary aspects of their guidelines that were mentioned in state of the art\cref{sec:soa_reproducibility} we propose an abstract model to represent the experiment, 
...
The orchestrator provides three intrafaces : 
