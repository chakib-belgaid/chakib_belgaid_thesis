...
\begin{itemize}
    \item is there a dominant  programing langauges when it comes to , energy consmption and latency ?
    \item which class of programming language is perfromring well ?
    \item is there a correlation between the energy consumption and the latency ?
    \item is there an impact of the server when it comes to change the data base ?
\end{itemize}
...
\subsection{overal all statistics}
In order to determine which framewrok/stack is perfroming well we need to establish some general idea about the average energy consumption and latency of the frameworks. Instead of giving just the raw energy consumption of those frameworks we will provide some green factors to determine which one is ecofriednly and which one is gluton.
In this part we will discuss the Average behaviour of the frameworks, highlight some tendecies and elimante the outliers.
As we said in the threats to validity, being an outlier in this case doenst mean that the framework is not performing well, it just means that the framework is not performing well in the same way as the others within the context of this experiment.
...
\begin{figure}[bht]
    ...
    \caption{total request vs average power consumption for the uptate queries test ( Size of circles represents the size of the query )}
