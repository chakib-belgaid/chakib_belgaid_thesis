...
\motivation{Comparison of Web Frameworks}
...
\begin{itemize}
    ...
    \item \textsf{recorder} collects the power measurements from the SUT and the key performance metrics collected by the clients \todo{add a link to the measurement process}.
\end{itemize}
...
In this benchmark, each response is a JSON serialization of a freshly-instantiated object that maps the critical message to the value \texttt{"Hello, World!"}.
For each of the above scenarios, we consider different stress levels, and Table~\ref{frameworks:stress-levels} shows the different levels for each scenario.
